  ...
  return (
    <li className="relative min-w-0">
      <StorefrontFavoriteButton
        ...
        className="absolute end-1.5 top-1.5 z-20 !h-7 !w-7"
      />
      <div
        className={[
          'sf-product-card-compact flex w-full flex-col overflow-hidden rounded-2xl bg-white ring-1 transition duration-300',
          available
      ...
      >
        <button
          type="button"
          onClick={onOpen}
          className="block w-full text-start active:scale-[0.98]"
        >
          <div className="relative aspect-square w-full overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100">
            {img ? (
              <img
                ...
                className={[
                  ...
                  available ? 'hover:scale-105' : 'grayscale-[0.5]',
                ].join(' ')}
                loading="lazy"
              />
            ) : (
            ...
            {onSale && available ? (
              <span className="absolute start-1.5 top-1.5 z-10 rounded-md bg-emerald-500 px-1.5 py-0.5 text-[9px] font-bold text-white shadow-sm">
                %
              </span>
            ) : null}
            {inCart > 0 && available ? (
              <span
                className="absolute bottom-10 end-1.5 z-10 flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[9px] font-extrabold text-white shadow"
          ...
          <p className="line-clamp-2 px-2 pb-2 pt-2 text-[11px] font-bold leading-snug text-slate-800">
            {product.name}
          </p>
        </button>
        {available ? (
