...
export type OutstandingDebtSaleLine = {
  receiptNumber: string
  occurredAt: string
  balanceUsd: number
}

export type OutstandingDebtReceiptData = {
  customerName: string
  customerPhone: string
  customerAddress: string
  customerId: number
  outstandingUsd: number
  outstandingIqd: number | null
  exchangeRate: number
  sales: OutstandingDebtSaleLine[]
}

export async function buildOutstandingDebtReceiptHtml(args: {
  data: OutstandingDebtReceiptData
  receiptSettings: ReceiptSettingsRow | null
}): Promise<string> {
  const { data, receiptSettings } = args
  const receiptQrBlock = await buildReceiptQrBlock(receiptSettings)
  ...
  const outstandingIqd =
    data.outstandingIqd != null && Number.isFinite(data.outstandingIqd)
      ? data.outstandingIqd
      : rate > 0
        ? data.outstandingUsd * rate
        : null
  ...
  const customerPhone = escapeHtml(data.customerPhone || '')
  const customerAddress = escapeHtml(data.customerAddress || '')
  const occurred = new Date().toLocaleString()
  const receiptNo = String(data.customerId).padStart(6, '0')

  const tableRows = data.sales
    .map((sale, idx) => {
      const occurredAt = sale.occurredAt ? new Date(sale.occurredAt).toLocaleString() : '—'
      return `<tr>
        <td class="cell-idx"><span class="cell-pad">${idx + 1}</span></td>
        <td class="col-name"><span class="cell-pad cell-name-text">${escapeHtml(sale.receiptNumber || '—')}</span></td>
        <td class="col-date"><span class="cell-pad">${escapeHtml(occurredAt)}</span></td>
        <td class="num cell-line-total"><span class="cell-pad">${fmtUsd(sale.balanceUsd)}</span></td>
      </tr>`
    })
    .join('')

  const outstandingIqdRow = showIqdOnPdf
    ? `<div class="tot-row tot-row--muted"><span>قەرزی ماوە (IQD)</span><span class="num">${outstandingIqd != null ? fmtIqd(outstandingIqd) : '—'}</span></div>`
    : ''
  ...
  const styles = `
        ...
        * { box-sizing: border-box; }
        ...
        .receipt-table col.col-idx { width: 7%; }
        .receipt-table col.col-name { width: 22%; }
        .receipt-table col.col-date { width: 43%; }
        .receipt-table col.col-total { width: 28%; }
        .receipt-table th,
        .receipt-table td { border: 1px solid #e5e7eb; padding: 0; }
        .receipt-table th {
          background: #e2e8f0;
        ...
        .cell-pad { display: block; padding: 6px; }
        .receipt-table .col-name { text-align: start; }
        .receipt-table .col-date { text-align: start; }
        .receipt-table .num { text-align: end; font-variant-numeric: tabular-nums; }
        .cell-idx { text-align: center; color: #475569; }
        .cell-name-text { font-weight: 600; }
        .receipt-table tbody tr:nth-child(odd) td { background: #f8fafc; }
  ...
  return `<!doctype html><html dir="rtl" lang="ku"><head><meta charset="utf-8" />
      ...
      <div class="receipt">
        <div class="invoice-hero">
          ...
          <div class="invoice-hero__badge">
            <p class="invoice-hero__badge-ku">INVOICE</p>
            <p class="invoice-hero__badge-en">پسوولەی قەرزی ماوە</p>
            <p class="invoice-hero__badge-id">[ ژمارە: ${escapeHtml(receiptNo)} ]</p>
          </div>
        </div>
        <div class="receipt-body">
        ...
        <p class="section-title">وردەکاری قەرز</p>
        <div class="receipt-table-wrap">
          <table class="receipt-table">
            <colgroup>
              <col class="col-idx" />
            ...
            <thead><tr>
              <th scope="col"><span class="th-inner">#</span></th>
              <th scope="col"><span class="th-inner">ژمارەی پسوولە</span></th>
              <th scope="col"><span class="th-inner">بەروار</span></th>
              <th scope="col"><span class="th-inner">قەرزی ماوە (USD)</span></th>
            </tr></thead>
            <tbody>${tableRows || '<tr><td colspan="4" class="cell-pad" style="text-align:center;padding:12px">—</td></tr>'}</tbody>
          </table>
        </div>
        <div class="receipt-totals">
          <div class="tot-row tot-row--balance"><span>قەرزی ماوە (USD)</span><span class="num">${fmtUsd(data.outstandingUsd)}</span></div>
          ${outstandingIqdRow}
          <div class="tot-row tot-row--strong"><span>کۆی گشتی قەرز (USD)</span><span class="num">${fmtUsd(data.outstandingUsd)}</span></div>
        </div>
        <div class="receipt-bottom">
          <div></div>
          <div class="sig">
